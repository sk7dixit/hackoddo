...
export const ActivityLogs: React.FC = () => {
  ...
  const filteredLogs = logs.filter((log) => {
    const userVal = log.user || log.operator || "System";
    const assetVal = log.asset || "";
    const moduleVal = log.module || "";
    const actionVal = log.action || "";

    const matchesSearch =
      userVal.toLowerCase().includes(search.toLowerCase()) ||
      assetVal.toLowerCase().includes(search.toLowerCase()) ||
      moduleVal.toLowerCase().includes(search.toLowerCase()) ||
    ...
    <div className="p-6 space-y-5 animate-fade-in pb-16">
      ...
      <div className="border-b border-slate-200/60 pb-4">
        ...
        <p className="text-sm text-slate-500 font-semibold mt-1">
          Immutable auditable activity logs for security, reporting, and
          tracking
        </p>
      </div>
              ...
              <tbody className="font-semibold text-slate-650">
                {currentItems.length === 0 ? (
                  ...
                  currentItems.map((log) => {
                    const badgeClass =
                      MODULE_BADGE_COLORS[log.module.toLowerCase()] ||
                      MODULE_BADGE_COLORS.system;
                    return (
                      <tr
                        key={log.id}
                      ...
                      >
                        ...
                        <td className="py-3.5 px-4 text-slate-700 flex items-center gap-1.5">
                          ...
                          <span>{log.user || log.operator || "System"}</span>
                        </td>
                        ...
                        <td className="py-3.5 px-4">
                          <span
                            className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold border ${badgeClass}`}
                          >
                            {log.module}
                          </span>
                        </td>
                        ...
                        <td className="py-3.5 px-4 font-mono font-bold text-[#5B5BD6]">
                          {log.asset || "System"}
                        </td>
